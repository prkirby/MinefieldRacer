	...
	private void switchModes(){
		...
		if(this.currentTime <= 0 && !inRace){
			...
			for(int c = 0; c < this.clients.size(); c++){
				...
				if(clients.get(c).getName().equals("")){
					System.out.println("enter name");
					Scanner scan = new Scanner(System.in);
					clients.get(c).setName(scan.next());
				}
			}
		}
		...
		try{
			for(int cl = 0; cl < clients.size(); cl++){
				if(!clients.get(cl).isActive()){
					...
					//client wins
					if(!this.someoneWon){
						if(clients.get(cl).player().getX() == map.getWidth() -3){//-3 due to the border and finish line size
							clients.get(cl).setWinner(true);
							System.out.println(clients.get(cl).getName() + " won!");
